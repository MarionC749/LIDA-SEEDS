...
land_cover_colours= {
    ...
    "Inland rock": "red",
    "Neutral grassland": "peru",
    "Suburban": "mediumvioletred",
    "Urban": "mediumorchid", 
}

soil_texture_colours= {
    "LIGHT(SILTY) TO MEDIUM(SILTY) TO HEAVY": "lightcoral",
    "MEDIUM TO HEAVY": "indianred",
    "LIGHT(SILTY) TO MEDIUM(SILTY)": "brown",
    "MEDIUM TO LIGHT(SILTY) TO HEAVY": "firebrick",
    "LIGHT(SANDY) TO MEDIUM(SANDY)": "maroon",
    "HEAVY TO MEDIUM": "darkred",
    "ALL": "red",
    "MEDIUM(SILTY)": "salmon",
    "MEDIUM": "tomato",
    "MEDIUM TO LIGHT(SILTY)": "coral",
}
...
def add_categorical_layer(fig, #map
    ...
    for category in categories:
        ...
        fig.add_trace(
            ...
            fillcolor= hex_to_rgba(colour, 0.4),
            name= category,
...
def dvpt_update_map_state(soil_layers,
    ...
    # ------ MAP CLICK ------
        ...
        #Clicking map stores centroid coordinates of polygon clicked
        point = clickData['points'][0]
        
        feature_id= (point['customdata'][0])
        
        feature= soil_health.loc[feature_id]
        
        centroid= (feature.geometry.centroid)
        
        dvpt_state['dvpt_selected_features']= feature_id
...
#Define what the state of the map should be
    ...
    dvpt_apply_zoom_logic(fig, postcode, sidebar)
...
# ------ Priority Zoom System ------
def dvpt_apply_zoom_logic(fig, postcode, sidebar):
    ...
    if postcode:
        ...
        if not row.empty:
            ...
            fig.update_layout(map=dict(center={'lat': centroid.y, 'lon': centroid.x}, zoom=15))
...
def dvpt_render_sidebar(dvpt_state):
    ...
    #Sidebar retrieves idx of clicked polygon
    feature_id= dvpt_state.get('dvpt_selected_features')
    ...
    if "Land Cover" in selected_layers:
        row= soil_health[
            (soil_health["Soil_Metric"]== "Land Cover")
            &
            (soil_health.index == feature_id)
        ]
    ...
    # SOIL PARENT
    if (
        "Soil Texture" in selected_layers
        or
        "Grain Size Class" in selected_layers
    ):
        row= soil_health.loc[feature_id]
    ...
    if "Soil pH" in selected_layers:
        row= soil_health[
            (soil_health["Soil_Metric"]== "Soil pH")
            &
            (soil_health.index == feature_id)
        ]
    ...
    if "Soil SOM" in selected_layers:
        row= soil_health[
            (soil_health["Soil_Metric"]== "Soil SOM")
            &
            (soil_health.index == feature_id)
        ]
